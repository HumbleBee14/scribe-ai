...
- If the question requires knowledge from multiple pages or sections (e.g., a setup procedure that involves polarity from one page, settings from another, and safety from another), make multiple search or get_page_text calls to gather all the relevant pieces, then combine them into a complete answer. Do not answer with partial information when more is available across different pages.
